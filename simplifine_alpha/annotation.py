...
class synthetic:
    ...
    async def create_qa_pair_single_async(
        self,
        text: str,
        chunk_size: int = 40,
        overlap: int = 10,
        ...
        **NOTE: this is a synchronous call, and it is not recommended to use it for large texts.
        ...
        if shrink:
            ...
            shrink_indices = random.sample(
                range(len(chunks)), int(ratio_of_shrink * len(chunks))
            )
            shrunk_chunks = [
                chunks[i] for i in range(len(chunks)) if i in shrink_indices
            ]
    ...
    def QA_gen_batch(
        ...
        shrink_indices, chunks = self.create_qa_pair_batch_file(
            text, file_name, chunk_size, overlap, shrink, ratio_of_shrink
        )
    ...
    def create_QA_annotation_file(
        self, batch_id: str, file_path: str, overwrite: bool = False
    ):
        """
        ...
        **NOTE: file type accepted now is only .jsonl
        ...
        if file_extension != "jsonl":
            raise ValueError(
                f"File type is {file_extension}, only jsonl is supported at the moment."
            )
        ...
        if job_task_dont_match:
            raise ValueError(
                f'Batch ID {batch_id} does not match the task, task recorded as: {all_meta_data[i]["task"]}, target should be: ANOT-QA'
            )
    ...
    def create_prompt_summarization(self, text: str):
        system_prompt = """You are a helpful assistant summarizing a text. You are given a text and you need to create a summary of it.
    ...
    def summarization_gen_batch(self, text: list, description=""):
        ...
        log_batch_metadata(
            ...
            file_name,
            description,
            task="ANOT-SUMMARIZATION",
            task_specific_paramters={},
        )

    def create_summarization_annotation_file(
        self, batch_id: str, file_path: str, overwrite: bool = False
    ):
        """
        retireve the results of a batch request and save it as a jsonl file.
        **NOTE: file type accepted now is only .jsonl
        ...
        if job_task_dont_match:
            raise ValueError(
                f'Batch ID {batch_id} does not match the task, task recorded as: {all_meta_data[i]["task"]}, target should be: ANOT-SUMMARIZATION'
            )
        matched_resp = match_response_to_request(batch_id, self.client)
    ...
    def check_batch_matches_task(self, batch_id: str, task: str):
        ...
        if job_task_dont_match:
            raise ValueError(
                f'Batch ID {batch_id} does not match the task, task recorded as: {all_meta_data[i]["task"]}, target should be: ANOT-CLASSIFICATION'
            )

    def create_clf_annotation_file(
        self, batch_id: str, file_path: str, overwrite: bool = False
    ):
        """
        retireve the results of a batch request and save it as a jsonl file.
    ...
    def function_calling_prompt(
        self,
        text: str,
        functions: dict = {"function": [], "parameters": [], "description": []},
    ):
        system_prompt = """You are a helpful assistant calling a function. You are given a text and you need to call the appropriate function. The user will provide you with the functions to choose from.
    ...
    # TODO: complete the function
    def function_calling_single(
        self, text: str, functions: dict = {"function": [], "description": []}
    ):
        pass
