...
def hf_finetune_llm_qa(
    ...
    if from_hf:
        ...
        dataset = Dataset.from_dict(
            {"questions": queries, "contexts": context, "answers": answers}
        )
...
# TODO: attend to this
def hf_finetune_embedder_contrastive(
    model_name: str,
    dataset_name: str = "",
    queries: list = [],
    positives: list = [],
    ...
    corpus = {
        "ids": range(len(positives) + len(negatives)),
...
def hf_finetune_embedder_positive(
    model_name: str,
    dataset_name: str = "",
    questions: list = [],
    docs: list = [],
    ...
    if from_hf:
        ...
        if do_split:
            ...
            questions, questions_eval = (
                questions[:-split_index],
                questions[-split_index:],
            )
            relevant_ids, relevant_ids_eval = (
                relevant_ids[:-split_index],
                relevant_ids[-split_index:],
            )
    ...
    train_dataset = Dataset.from_dict(
        {"anchor": questions, "positive": [docs[i] for i in relevant_ids]}
    )
    print("\n-----------------------------------------\n")
    print(len(questions))
...
def sft_train(
    ...
    if system_message_key and system_message:
        raise ValueError(
            "Only provide key from dataset or system message as a string, not both"
        )
    ...
    if use_ddp and sft_config.gradient_checkpointing:
        print(
            "[WARNING]: Gradient checkpointing is not supported with DDP. Disabling gradient checkpointing."
        )
    ...
    if use_chat_template:
        if tokenizer.chat_template:
            ...
            chat_temp = text.split(messages[1]["content"])[-1].split(
                messages[-1]["content"]
            )[0]
        ...
        try:
            if dataset_config_name:
                raw_datasets = load_dataset(
                    dataset_name, dataset_config_name, token=False, split="train"
                )
            else:
            ...
            if dataset_config_name:
                raw_datasets = load_dataset(
                    dataset_name, dataset_config_name, token=False
                )
            else:
    ...
    if tokenizer.chat_template:
        collator = DataCollatorForCompletionOnlyLM(
            chat_response_temp, tokenizer=tokenizer
        )
    else:
        collator = DataCollatorForCompletionOnlyLM(
    ...
    promptTokenizedDataset = raw_datasets.map(
    ...
    print(
        f"train data set is: {promptTokenizedDataset['train']}, eval dataset is {promptTokenizedDataset['test']}"
    )
    if do_split:
        trainer = SFTTrainer(
            model,
...
def hf_sft(
    ...
    def formatting_prompts_func(example):
        ...
        tokenized_output = tokenizer(
            output_texts, truncation=True, padding="max_length", add_special_tokens=True
        )
    ...
    if data_from_hf:
        try:
            if dataset_config_name:
                ...
                raw_datasets = raw_datasets.rename_column(
                    prompt_config.clm_column, "text"
                )
            else:
                raw_datasets = load_dataset(dataset_name, token=False, split="train")
                raw_datasets = raw_datasets.rename_column(
...
def hf_clf_train(
    ...
    if report_to == "wandb":
        wandb.login(key=wandb_api_key)
        wandb.init(
            project="clm_train", config={"model_name": model_name, "epochs": num_epochs}
        )
