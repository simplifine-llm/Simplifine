...
def retireve_batch_results(client, batch_id):
    ...
    if status_meta_data.status == "completed":
        content = client.files.content(status_meta_data.output_file_id).content.decode(
            "utf-8"
        )
        return content
    else:
...
def update_batch_status(client):
    ...
    for i in range(len(all_meta_data)):
        if (
            all_meta_data[i]["status"] != "completed"
            or all_meta_data[i]["status"] != "failed"
            or all_meta_data[i]["status"] != "canceled"
            or all_meta_data[i]["status"] != "expired"
        ):
            all_meta_data[i]["status"] = get_batch_status(
                client, all_meta_data[i]["id"]
            )
