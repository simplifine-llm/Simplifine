...
def parse_sft_prompt(
    generate_config: GenerationConfig, data: dict, tokenizer: AutoTokenizer
):
    _is_chat = False
    ...
    for i in range(len(data[generate_config.keys[0]])):
        ...
        if _is_chat:
            user_text, assistant_text = formatted_text.split(
                generate_config.response_template
            )
            ...
            chat_prompt_pre_gen, chat_prompt_post_gen = chat_prompt.split(
                chat_response_temp
            )
            formatted_prompts.append(chat_prompt_pre_gen)
            expected_responses.append(chat_prompt_post_gen)
        else:
            formatted_text_pre_gen, formatted_text_post_gen = formatted_text.split(
                generate_config.response_template
            )
...
def generate_from_pretrained(
    ...
    if generate_confg.train_type == "sft":
        formatted_prompt, expected_outputs = parse_sft_prompt(
            generate_confg, data, tokenizer
        )
        for prompt, expected_output in zip(formatted_prompt, expected_outputs):
            inputs = tokenizer(prompt, return_tensors="pt")
            outputs = model.generate(
                **inputs,
                max_length=generate_confg.max_length,
                num_return_sequences=generate_confg.num_return_sequences,
                do_sample=generate_confg.do_sample,
                top_k=generate_confg.top_k,
                top_p=generate_confg.top_p,
                temperature=generate_confg.temperature,
            )
