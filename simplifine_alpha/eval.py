...
if __name__ == "__main__":  
    eval = eval_gen()
    # question = ["What is the capital of France?", "What is the capital of Germany?", "What is the capital of Italy?"]
    # answers = ["Paris", "Berlin", "Rome"]
    # eval.QA_eval_batch(question, answers, give_explanation=True, context=True)
    # batch_id = 'batch_vPJdGAyVqZ9zruovZG4egmLR'
    # update_batch_status(eval.client)
    # print(eval.QA_eval_batch_get_metrics(batch_id))
    # print('\n----\n')
    # batch_id = "batch_Xj2Bmjkb4S7CD4jKNwyiMUnJ"
    # print(retireve_batch_results(eval.client, batch_id))
    # print(eval.parse_async_eval_hallucination_rag(batch_id))

    # queries = ['a','b','c','d']
    # labels = ['a','b','c','d']
    # results = eval.eval_embedder(queries=queries, context=labels, label_provided=True)
    # print(results)

    # text = "This is a test text to be chunked into smaller parts."
    queries = ['a','b','c','d']
    chunks = [['a','b','c'],['d','e','f'],['g','h','i'],['j','k','l']]
    print(eval.sync_embed_eval_per_ctx(queries=queries, chunks=chunks, relevant_chunks_provided=True))
    # a= eval.async_embed_eval(text, queries, top_k=2, use_faiss=False)
    # print(a)
    # batch_id = 'batch_u2nwYHfKsIST2h64N8j0lsCC'
    # print(eval.sync_embed_eval(text, queries, top_k=2, use_faiss=False))
    # queries = ['What is the capital of France?', 'What is the capital of Germany?', 'What is the capital of Italy?']
    # contexts = [['Paris has been the capital of France since the 5th century.', 'The Eiffel Tower is located in Paris.', 'The Louvre Museum is located in Paris.', 'The Seine River runs through Paris.'],
                
    #             ['Berlin is the capital of Germany.', 'The Brandenburg Gate is located in Berlin.', 'The Berlin Wall divided the city of Berlin.', 'Berlin is the largest city in Germany.'],
                
    #             ['Rome is the capital of Italy.', 'The Colosseum is located in Rome.', 'The Vatican City is an independent city-state within Rome.', 'Rome is the largest city in Italy.']]
    
    # answers = ['Paris', 'A good horse is a good horse.', 'Rome']
    # print(eval.eval_hallucination_rag_async(queries, contexts, answers, num_outputs=5, based_on_context=True))
