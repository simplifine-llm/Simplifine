...
class eval_gen:
    ...
    def create_prompt_single_qa(
        ...
        system_prompt = """You are a helpful assistant assessing if an answer is correct or not.
    ...
    def QA_eval_single(
        ...
        messages = self.create_prompt_single_qa(
            question, answer, give_explanation, context, example
        )
    ...
    def create_batch_file_QA(
        ...
        context: bool,
        example=None,
        file_name="batch_request_train.jsonl",
    ):
        """
        ...
        for q, a, reqID in zip(questions, answers, request_ids):
            batch_format = {"custom_id": "", "method": "", "url": "", "body": {}}
            messages = self.create_prompt_single_qa(
                q, a, give_explanation, context, example
            )
    ...
    def QA_eval_batch(
        ...
        self.create_batch_file_QA(
            questions, answers, give_explanation, context, example, file_name
        )
    ...
    def QA_eval_batch_get_metrics(self, batch_id: str):
        ...
        if task != "EVAL-QA":
            raise ValueError(
                f"The target task should be EVAL-QA, but it is recorded as {task}"
            )
    ...
    def summary_eval(
        ...
        input_text,
        generated_summary,
        metric_meta_data={"metric_name": [], "metric": [], "metric_scoring": []},
    ):
        """
        ...
        **NOTE: This is a synchronus function and is not suitable for batch requests.
        ...
        for i in range(len(metric_meta_data["metric_name"])):
            summary_eval_prompt = self.summary_eval_prompt.format(
                criteria=metric_meta_data["metric"][i],
                steps=metric_meta_data["metric_scoring"][i],
                document=input_text,
                summary=generated_summary,
                metric_name=metric_meta_data["metric_name"][i],
            )
            messages = [
    ...
    def summary_eval_batch(
        ...
        for text, summ, ind in zip(
            input_texts, generate_summaries, range(len(input_texts))
        ):
        ...
        batch_input_file = self.client.files.create(
            file=open(save_path, "rb"), purpose="batch"
        )
    ...
    def eval_embedder_with_pairs(
        ...
        if self.device_name == "cuda":
            ...
            model = SentenceTransformer(
                embedder, device=self.device, trust_remote_code=True
            )
    ...
    def eval_embedder_openAI_prompt_creation_per_ctx(
        self,
        text: str = "",
        queries: list = [],
        embedder: str = "paraphrase-MiniLM-L6-v2",
        ...
        system_prompt = """You are a helpful assistant assessing if the context provided by the user is relevant to the query.
    ...
    def sync_embed_eval_per_ctx(
        ...
        """
        user_prompts, system_prompt = self.eval_embedder_openAI_prompt_creation_per_ctx(
            text,
            queries,
            embedder,
            chunk_size,
    ...
    def async_embed_eval_per_ctx(
        ...
        returned_json = self.client.batches.create(
            ...
            endpoint="/v1/chat/completions",
            completion_window="24h",
            metadata={"description": "Evaluation of an embedder per context provided"},
        )
        task_specific_paramters = {
            "embedder": embedder,
    ...
    # TODO: faiss not working, fix it!!!
    def eval_embedder_openAI_prompt_creation(
        self,
        text: str = "",
        queries: list = [],
        embedder: str = "paraphrase-MiniLM-L6-v2",
    ...
    def sync_embed_eval(
        ...
        matched_response: dict, the matched response from the model
        """
        user_prompts, system_prompt = self.eval_embedder_openAI_prompt_creation(
            text,
            queries,
            embedder,
            chunk_size,
    ...
    def async_embed_eval(
        ...
        returned_json = self.client.batches.create(
            ...
            endpoint="/v1/chat/completions",
            completion_window="24h",
            metadata={"description": "Evaluation of an embedder"},
        )
        task_specific_paramters = {
            "text": text,
            "queries": queries,
    ...
    def eval_hallucination_rag_prompt(
        self,
        query: str,
        contexts: str,
        answer: str,
        ...
        """
        system_prompt = """You are a helpful assistant assessing if an answer given to a question based on context includes hallucinations or not.
    ...
    def eval_hallucination_rag_sync(
        self,
        query: str,
        contexts: str,
        answer: str,
        ...
        """
        messages = self.eval_hallucination_rag_prompt(
            query, contexts, answer, num_outputs, based_on_context
        )
        completion = self.client.chat.completions.create(
            model=self.model_name, messages=messages
        )
    ...
    def eval_hallucination_rag_async(
        self,
        queries: list,
        contexts: list,
        answers: list,
        ...
        for num, (query, context, answer) in enumerate(zip(queries, contexts, answers)):
            messages = self.eval_hallucination_rag_prompt(
                query, context, answer, num_outputs, based_on_context
            )
            batch_format = {"custom_id": "", "method": "", "url": "", "body": {}}
            batch_format["custom_id"] = f"request-{num}"
            batch_format["method"] = "POST"
        ...
        returned_json = self.client.batches.create(
            ...
            endpoint="/v1/chat/completions",
            completion_window="24h",
            metadata={"description": "Evaluation of hallucination in a RAG pipe"},
        )
        task_specific_paramters = {}
        # save the metadata for the batch request
        log_batch_metadata(
            ...
            batch_input_file_id,
            file_name,
            "Evaluation of hallucination in a RAG pipe",
            task="EVAL-HALLUCINATION-RAG",
            task_specific_paramters=task_specific_paramters,
        )
    ...
    def parse_async_eval_hallucination_rag(self, batch_id: str = ""):
        ...
        if task != "EVAL-HALLUCINATION-RAG":
            raise ValueError(
                f"Task is not EVAL-HALLUCINATION-RAG, instead it is {task}"
            )
